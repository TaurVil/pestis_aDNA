...
```console
# index genome
# indexing needs to be done with same bwa version as mapping: /project2/lbarreiro/users/tauras/Programs/bwa_bam2bam/network-aware-bwa-master/bwa index /project2/lbarreiro/users/tauras/my_genomes/hg19/hg19.fa
...
for index in `seq 335 336`; do f=`head -$index 00_bam.list | tail -1`;
	...
	# map 
	/project2/lbarreiro/users/tauras/Programs/bwa_bam2bam/network-aware-bwa-master/bwa bam2bam -g /project2/lbarreiro/users/tauras/my_genomes/hg19/hg19.fa -n 0.01 -o 2 -l 16500 -f bams/$f.remapped.bam tmp6.$f.tomap.bam
...
for index in `seq 1 364`; do f=`head -$index 00_sample.list | tail -1`; samtools merge ./bams/merged.$f.bam ./bams/$f[a,R,_]*.remapped.sort.bam ; samtools index ./bams/merged.$f.bam; echo $f; done
## check that we don't have anything greater than 3: for index in `seq 1 364`; do f=`head -$index 00_sample.list | tail -1`; ls ./bams/$f[a,R,_]*.remapped.sort.bam | wc -l | grep -v 3 | grep -v 2; done
```
...
# Again, we'll mix this between batch submitted and interactive jobs
	...
	cd ./bams/
	for index in `seq 111 120`; do f=`head -$index ../00_sample.list | tail -1`; mapDamage -i merged.$f.bam -r /project2/lbarreiro/users/tauras/my_genomes/hg19/hg19.fa --rescale; echo $index $f; done 
...
mkdir bams_trim4
for f in `cat 00_mapDamage_bams.list`; do /project2/lbarreiro/users/tauras/Programs/bamUtil/bam trimBam bams_mapDamage/merged.$f.rescaled.bam bams_trim4/$f.bam -L 4 -R 4; echo $f; done
